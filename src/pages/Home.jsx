...
function Home() {
    ...
    const loadData = async () => {
      try {
        const [geoRes, histRes] = await Promise.all([
          ...
          axios.get("https://ip-geo-web-backend.vercel.app/api/history", authHeaders),
        ]);
        if (isMounted) {
  ...
  const refreshHistory = async () => {
    const histRes = await axios.get(
      "https://ip-geo-web-backend.vercel.app/api/history",
      authHeaders,
    );
  ...
  const deleteSelected = async () => {
    ...
    try {
      await Promise.all(
        toDelete.map((item) =>
          axios.delete(
            `https://ip-geo-web-backend.vercel.app/api/history/${item.id}`,
            authHeaders,
          ),
        ),
      );
